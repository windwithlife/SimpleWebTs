...
const fecha = tslib_1.__importStar(require("fecha"));
...
function setFilePathForWinston(nowDateOfToday) {
    switch (NODE_ENV) {
        case 'production':
            winston_log_path_prefix = deploy_config_json_1.default.production.winston_log_path_prefix;
        ...
        // @ts-ignore
        case 'test':
            winston_log_path_prefix = deploy_config_json_1.default.mockpro.winston_log_path_prefix;
            error_log_file_path = `${winston_log_path_prefix}error/error_${nowDateOfToday}.log`;
            info_log_file_path = `${winston_log_path_prefix}info/info_${nowDateOfToday}.log`;
            exception_log_file_path = `${winston_log_path_prefix}exception/exception_${nowDateOfToday}.log`;
            rejection_log_file_path = `${winston_log_path_prefix}rejection/rejection_${nowDateOfToday}.log`;
...
/**
  ...
  */
function dailyRotateFileChild() {
    const timeSchedule = child_process_1.fork(path_1.default.join(__dirname, "./daily_rotate_file_child"));
    timeSchedule.send('start child process for update nowDateOfToday variable.');
    // The 'close' event is emitted when the stdio streams of a child process have been closed. This is distinct from the 'exit' event, since multiple processes might share the same stdio streams.
    timeSchedule.on('close', (code, signal) => {
     ...
     */
    timeSchedule.on('exit', (code, signal) => {
 ...
 * 添加transport
 */
function addYourTransport() {
    logger.clear(); //remove all transports
    const commonFormat = format.combine(format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS A' }), winston_1.default.format.json());
    const fileFormatOfErrorLevel = new winston_1.default.transports.File({
        level: "error",
        filename: `${error_log_file_path}`,
        format: commonFormat
    });
    const fileFormatOfInfoLevel = new winston_1.default.transports.File({
        level: "info",
        filename: `${info_log_file_path}`,
        format: commonFormat
    });
    logger.exceptions.handle(new transports.File({ filename: exception_log_file_path }));
    //@ts-ignore
    logger.rejections.handle(new transports.File({ filename: rejection_log_file_path }));
    // add transport
